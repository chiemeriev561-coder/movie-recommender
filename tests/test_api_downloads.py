...
def test_direct_download_without_api_keys(monkeypatch):
    monkeypatch.setattr(api, "TORBOX_API_KEY", "")
    monkeypatch.setattr(api, "REAL_DEBRID_API_KEY", "")

    response = client.get("/api/movies/550/direct-download?quality=1080p")
    assert response.status_code == 200
    data = response.json()
    assert data["available"] is False
    assert "TORBOX_API_KEY" in data["message"]

def test_direct_download_with_torbox_key(monkeypatch):
    monkeypatch.setattr(api, "TORBOX_API_KEY", "test_torbox_token")
    monkeypatch.setattr(api, "REAL_DEBRID_API_KEY", "")
                    ...
                    "torrents": [
                        {
                            ...
                            "peers": 30,
                            "hash": "HASH123",
                            "url": "https://yts.gg/torrent/download/HASH123"
                        }
                    ]
                }
            ]
    ...
    class MockAsyncClient:
        ...
        async def get(self, url, **kwargs):
            ...
            if "requestdl" in url:
                mock_resp.status_code = 200
                mock_resp.json.return_value = {"success": True, "data": "https://torbox.app/d/fightclub.mp4"}
            else:
                mock_resp.status_code = 200
                mock_resp.json.return_value = mock_yts_response
            return mock_resp
        async def post(self, url, **kwargs):
            mock_resp = MagicMock()
            mock_resp.status_code = 200
            mock_resp.json.return_value = {"success": True, "data": {"torrent_id": 999}}
            return mock_resp

    monkeypatch.setattr(api.httpx, "AsyncClient", MockAsyncClient)

    response = client.get("/api/movies/tt0137523/direct-download?quality=1080p")
    assert response.status_code == 200
    data = response.json()
    assert data["available"] is True
    assert data["download_url"] == "https://torbox.app/d/fightclub.mp4"
    assert data["provider"] == "Torbox"
